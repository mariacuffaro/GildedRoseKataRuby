...
describe GildedRose do
  ...
  describe "#update_quality" do
    ...
    xit "reduces the sellin value of the input items" do
      items = [Item.new("item", 3, 2)]
      GildedRose.new(items).update_quality()
      expect(items[0].sell_in).to eq(2)
    end
    xit "reduces the quality value of the input items" do
      items = [Item.new("item", 3, 2)]
      GildedRose.new(items).update_quality()
    ...
    xit 'never reduces the quality of an item once it is zero' do
      items = [Item.new("standard_item", 3, 0)]
      GildedRose.new(items).update_quality()
      expect(items[0].quality).to eq(0)
    end
    xit 'increases the quality of Aged Brie items' do
